...
REPO='reno-test-rahul/repro-33701'
...
if [ "$ISSUES_MISSING_LABEL" != "[]" ]; then
    ...
    FORMATTED_OUTPUT=$(echo "$ISSUES_MISSING_LABEL" | jq -r '.[].number' | sed 's/^/- https:\/\/redirect.github.com\/reno-test-rahul\/repro-33701\/issues\//')
...
if [ "$ISSUES_MISSING_TYPE" != "[]" ]; then
    ...
    # Create a list of issue numbers
    FORMATTED_OUTPUT=$(echo "$ISSUES_MISSING_TYPE" | jq -r '.[].number' | sed 's/^/- https:\/\/redirect.github.com\/reno-test-rahul\/repro-33701\/issues\//')
